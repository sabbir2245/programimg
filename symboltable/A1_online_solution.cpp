...
public:
    ...
    void moveAllSymbolsFrom(ScopeTable* child) {
        for (int i = 0; i < total_buckets; i++) {
            child->getBucketByIndex(i).for_each([&](SymbolInfo& si) {
                table[i].push_back(SymbolInfo(si.getName(), si.getType(), si.getExtraInfo()));
            });
        }
    }
...
// =============================new code =======================
    bool Merge() {
        ...
        for (int i = 0; i < bucketSize && !conflict; i++) {
            child->getBucketByIndex(i).for_each([&](SymbolInfo& si) {
                if (!parent->canInsert(si.getName())) {
                    out << "\tMerge failed: '" << si.getName() << "' already exists in parent ScopeTable" << endl;
                    conflict = true;
                }
            });
        }
        ...
        currentScope = parent;
        out << "\tScopeTable# " << child->getScopeId() << " merged with ScopeTable# " << parent->getScopeId() << endl;
        delete child;
        return true;
    }
// =============================new code =======================
...
int main(int argc, char* argv[]) {
...

    {
        ...
        while (getline(inFile, line)) {
            ...
            } else if (cmd != "I" && cmd != "L" && cmd != "D" && cmd != "S" && cmd != "E" && cmd != "M") {
                continue;
            }
